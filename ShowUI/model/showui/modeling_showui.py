...
class ShowUIForConditionalGeneration(ShowUIPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        n_text_tokens, n_image_tokens, n_video_tokens = 0, 0, 0
        ...
        if inputs_embeds is None:
            ...
            # 统计文本token数量
            mask = (input_ids != self.config.image_token_id) & (input_ids != self.config.video_token_id)
            n_text_tokens = mask.sum().item()
        ...
        def ensure_tensor(x):
            ...
            if isinstance(x, torch.Tensor):
                return x.detach().clone().item()
            return torch.tensor(x)

        n_text_tokens = ensure_tensor(n_text_tokens)
        n_image_tokens = ensure_tensor(n_image_tokens)
        n_video_tokens = ensure_tensor(n_video_tokens)
