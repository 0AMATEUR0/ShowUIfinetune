...
# --- 1. 配置 ---
# 你的原始图片所在的文件夹
SOURCE_IMAGE_DIR = "D:/Project/my_dataset/unlabel_images"
# 你想生成的训练集文件夹名称
OUTPUT_DATASET_NAME = "my_dataset/screenspot"
# --- 结束配置 ---
...
def main():
    ...
    for image_file in image_files:
        ...
        print("3. 在控制台输入说明文字，然后选择类型 (text/icon)。")
        ...
        while True:
            ...
            ids = "screenspot_" + str(len(final_dataset))
            # 按 's' 保存当前画的框
            if key == ord('s'):
                if current_box:
                    task = input(">> 请输入这个框的说明 (task): ")
                    if not task:
                    ...
                    # 增加了选择类型的逻辑
                    data_type = ""
                    while data_type not in ["text", "icon"]:
                        type_choice = input(">> 这是 'text' 还是 'icon'? (输入 t 或 i): ").lower()
                        if type_choice == 't':
                            data_type = "text"
                        elif type_choice == 'i':
                            data_type = "icon"
                        else:
                            print("  [错误] 无效输入，请输入 't' (代表text) 或 'i' (代表icon)。")
                    # ########################## MODIFICATION END ##########################
                    ...
                    current_image_data["task"] = task
                    current_image_data["bbox"] = norm_bbox
                    current_image_data["data_type"] = data_type 
                    final_dataset.append(current_image_data)
                    # 在图像上用绿色永久标记已保存的框和文字
                    cv2.rectangle(img, (px_min, py_min), (px_max, py_max), (0, 255, 0), 2)
                    cv2.putText(img, f"#{current_image_data['id']}:{data_type}", (px_min, py_min - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                    img_display = img.copy()

                    print(f"  [成功] 框 #{current_image_data['id']} ({data_type}) 已保存: '{task}'")
                    current_box = None
                else:
                    print("  [提示] 请先画一个框再按 's'。")
            ...
            # 按 'n' 进入下一张图片
                ...
                if current_image_data is not None:
                    print(f"\n图片 {image_file} 完成，共 {current_image_data['id']} 个标注。")
                else:
            ...
            # 按 'q' 退出整个程序
                ...
                output_json_path = os.path.join(output_json_dir, "metadata.json")
